...
(define (inverter in out)
  (define (invert-input)
    (let ((new-val (logical-not (get-signal in))))
      (after-delay inverter-delay
                   (lambda () (set-signal! out new-val)))))
...
;;; connect-wire isn't from the book
(define (connect-wire a b)
  (add-action! a (λ () (set-signal! b (get-signal a))))
  (add-action! b (λ () (set-signal! a (get-signal b))))
  'ok)
...
(define (adder a1 a2 sum)
  ...
  (define (me request)
    (cond [(eq? request 'i-have-a-value)
          ...
          [else (error "unkonwn request -- adder" request)]))
...
(define (multiplier m1 m2 product)
  ...
  (define (me req)
    (cond [(eq? req 'i-have-a-value)  (process-new-value)]
          ...
          [else (error "unkonwn request -- multiplier" req)]))
...
(define (constant value connector)
  (define (me req)
    (error "unknown req -- constant" req))
...
(define (probe-constraint name connector)
  ...
  (define (me req)
    (cond [(eq? req 'i-have-a-value)  (process-new-value)]
          [(eq? req 'i-lost-my-value) (process-forget-value)]
          [else (error "unknown request -- probe" req)]))
  (connect connector me)
  me)
...
;; 3.35 complete the correct squarer
