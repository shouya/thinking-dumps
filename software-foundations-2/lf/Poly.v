...
Theorem rev_l_x : forall X: Type, forall x xs,
    rev (xs ++ [x]) = @cons X x (rev xs).
Proof.
  intros. induction xs.
  + reflexivity.
...
Proof.
  ...
  induction l.
  - reflexivity.
  - simpl. rewrite rev_l_x. rewrite IHl. reflexivity.
Qed.
(** [] *)
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun x => evenb x && ltb 7 x) l.
...
Lemma map_app_distr : forall (X Y : Type)
                             (f : X -> Y)
                             (xs ys: list X),
...
(** **** Exercise: 2 stars, standard, optional (implicit_args)
    ...
    This problem is pretty easy to me. So I'm going to skip it.
*)
(** [] *)
...
(* As a programmer by trade, I know a lot of such use cases in
practice.

 Here's a simple example. The following non-recursive definition of
 app takes advantage of that. *)

Definition app' {X : Type} (xs ys : list X) : list X :=
  fold cons xs ys.
...
Example fold_map_example_1 : fold_map S [1;2;3] = [2;3;4].
Proof. reflexivity. Qed.
...
Definition to_nat (x : cnat) : nat := x nat S 0.
Fixpoint from_nat (x : nat) : cnat :=
  match x with
  | O => zero
  | S x' => succ (from_nat x')
  end.
