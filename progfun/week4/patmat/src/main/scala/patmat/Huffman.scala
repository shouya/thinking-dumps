...
object Huffman {
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    (freqs.sortBy (frq => frq._2)).map(frq => Leaf(frq._1, frq._2))
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    val freq_lst = times(chars)
    val ordered_lst = makeOrderedLeafList(freq_lst)
    val codetree = until(singleton, combine)(ordered_lst)
    codetree.head
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeChar(subtree: CodeTree, bits: List[Bit]) : (Char, List[Bit]) =
	  ...
	  case xs      => throw new Exception("bad coding")
        }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    table.find(x => x._1 == char) match {
      case Some((_,bs)) => bs
      case None         => throw new Exception("Code Not found")
    }
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    tree match {
      ...
      case Fork(l,r,_,_) =>
        convert(l).map(a => (a._1,0::a._2)) :::
        convert(r).map(a => (a._1,1::a._2))
    }
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def find = codeBits(convert(tree))_
    text match {
      case c :: cs => find(c)::: encode(tree)(cs)
      case _       => List()
    }
  }
}
